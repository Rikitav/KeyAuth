...
/// <summary>
/// Ported from Java to C# by Hans Wolff, 10/10/2013.
/// Refactored by Rikitav, 31/01/2026.
...
internal class Ed25519
{
    ...
    private static readonly Dictionary<BigInteger, BigInteger> InverseCache = [];

    /// <summary>
    /// Encodes a BigInteger to a byte array.
    /// </summary>
    /// <param name="y">The BigInteger to encode.</param>
    ...
    /// <returns>The encoded point as a byte array.</returns>
    ...
    /// <summary>
    /// Generates a public key from a signing key.
    /// </summary>
    ...
    public static bool CheckValid(byte[] signature, byte[] message, byte[] publicKey)
    {
        ...
        byte[] rByte = signature.CopyOfRange(0, BitLength / 8);

        var r = DecodePoint(rByte);
        var a = DecodePoint(publicKey);

        byte[] sByte = signature.CopyOfRange(BitLength / 8, BitLength / 4);
